...
export default {
  ...
  methods: {
    ...
    handleEdit(id, row) {
        this.form = row;
        axios.get("http://169.254.239.219:9000/rolerList?id=" + id)
        .then(res => {
            console.log("res...", res);
            if (res.data.code == 0) {
    ...
    addTag(item){
        ...
        axios.get(`http://169.254.239.219:9000/addRoler?uid=${this.form.id}&rid=${item.id}`)
        .then(res => {
            console.log("res...", res);
            if (res.data.code == 0) {
                // this.allRoler = res.data.allRoler;
    ...
    removeTag(item, index){
        axios.get(`http://169.254.239.219:9000/removeRoler?uid=${this.form.id}&rid=${item.rid}`)
        .then(res => {
            console.log("res...", res);
            if (res.data.code == 0) {
                // this.allRoler = res.data.allRoler;
